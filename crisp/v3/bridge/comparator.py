...
_DEFAULT_EQUIVALENT_QUANTITATIVE_VALUES = {
...
class BridgeComparator:
        ...
        *,
        ...
        for channel_name in (PATH_CHANNEL_NAME,):
            ...
            drifts.extend(
                self._compare_path(
                    channel_name=channel_name,
                    rc2_observation=rc2_observation,
                    v3_observation=v3_observation,
                )
            )
    ...
    def _compare_path(
        ...
        *,
        ...
        for section_name in ("quantitative_metrics", "exploration_slice"):
            ...
            if section_name == "quantitative_metrics":
                rc2_keys, v3_keys = self._normalize_quantitative_metric_keys(
                    rc2_section=rc2_section,
                    v3_section=v3_section,
                )
            if rc2_keys != v3_keys:
                drifts.append(
                    DriftRecord(
    ...
    def _normalize_quantitative_metric_keys(
        self,
        *,
        rc2_section: dict[str, Any],
        v3_section: dict[str, Any],
    ) -> tuple[set[str], set[str]]:
        rc2_keys = set(rc2_section)
        v3_keys = set(v3_section)
        for key, default_value in _DEFAULT_EQUIVALENT_QUANTITATIVE_VALUES.items():
            if key not in rc2_keys and key in v3_keys and v3_section.get(key) == default_value:
                v3_keys.remove(key)
            elif key in rc2_keys and key not in v3_keys and rc2_section.get(key) == default_value:
                rc2_keys.remove(key)
        return rc2_keys, v3_keys
