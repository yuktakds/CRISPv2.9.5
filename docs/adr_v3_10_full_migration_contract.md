...
| WP-5 | CI-only / policy | exploratory → required promotion gate automation | landed as exploratory candidacy automation; required promotion remains explicit |
| RP-3A | docs-only human decision | operator-facing `v3_shadow_verdict` / numeric verdict-metric activation surfaces | accepted |
| RP-3B | docs-only human decision | required-promotion decision surface | accepted |
| RP-4 | rendering / hardening | operator-surface gate wiring and derived suppression / promotion state recording | landed without boundary change |
