...
current sidecar では `sidecar_run_record.json` が canonical Layer 0 record であり、`generator_manifest.json` が canonical sidecar inventory かつ re-materialization contract である。`output_inventory.json` は rc2 authority のまま不変である。`verdict_record.json` は current scope では schema reservation only, non-canonical, optional である。full-channel bridge により v3_shadow_verdict が正規定義された後にのみ、verdict_record.json の Layer 0 authority 昇格が検討対象になる。
...
| `sidecar_run_record.json` | 0 | sidecar state / comparator status / current authority | canonical current | current full-migration gate record |
...
| `verdict_record.json` | 0 candidate | future full verdict record | schema reservation only | non-authoritative until VN-06 satisfied |
...
Proposed branch: `docs/v3-full-migration-contract-adr`  
Proposed commit: `docs(v3): accept ADR-V3-10 full migration contract boundary and promotion policy`
