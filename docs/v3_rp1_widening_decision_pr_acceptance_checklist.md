...
以下のいずれかに `no` が残る場合、PR は merge してはならない。

1. widening target は `path_and_catalytic_partial` に限定されているか
2. `comparable_channels` widening は `["path", "catalytic"]` に限定されているか
3. Catalytic public meaning は Rule3A-only comparable projection で一意か
4. Rule3B `[v3-only]` 分離は cross-artifact で守られているか
5. `v3_shadow_verdict` / numeric `verdict_match_rate` の activation は完全に別 decision として隔離されているか
6. Cap exclusion は current authority と矛盾なく維持されているか

---

## Explicit Non-Authorization Statement

RP-1 widening decision PR の merge は、Catalytic Rule3A public comparable widening だけを authorize する可能性がある。  
それは次を authorize しない。
...
- required promotion
- Cap inclusion
- full-scope comparator widening

これらはすべて別 decision として残る。
