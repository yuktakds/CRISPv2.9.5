...
| `run_drift_report.json` | activation / promotion state section を追加（後述） | Layer 1 |
...
`run_drift_report.json` に以下の section を追加する。この section は operator summary の補助情報ではなく、Layer 1 machine-readable record として suppression audit に使う。
...
operator summary にはこの section の要約を `[exploratory]` label 付きで表示してよいが、authority はこの machine-readable record にある。
