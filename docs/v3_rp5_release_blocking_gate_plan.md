...
- public comparable component keys are `path`, `catalytic_rule3a`
- `v3_shadow_verdict` is inactive in the current public partial scope
- `verdict_match_rate` / `verdict_mismatch_rate` are still `None` or `N/A`
- rc2 remains primary; v3 remains `[exploratory]` secondary
- `path_component_match_rate` and `catalytic_rule3a` component match remain component-level indicators, not verdict proxies
