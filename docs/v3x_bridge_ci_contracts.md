...
Parent: `v3x_evidence_channel_kernel_architecture.md` §I ADR Queue  
...
class CompoundDriftReport:
    ...
    molecule_id: str
    comparator_scope: str       # "path_only_partial" / "full"
    rc2_verdict: str            # PASS / FAIL / UNCLEAR

    # SCV-level verdict（full scope でのみ non-None）
    v3_shadow_verdict: str | None
    verdict_match: bool | None  # full scope でのみ計算可能

    # component-level comparison（comparable channel ごと）
    component_matches: dict[str, bool | None]
    # 例: {"path": True}  — Path-only scope では path のみ
    #      full scope では全 channel を含む

    # comparability level（channel ごと）
    component_comparability: dict[str, str]
    # 例: {"path": "component_verdict_comparable"}
...
class RunDriftReport:
    ...
    # SCV-level verdict match（full scope でのみ有効）
    verdict_match_count: int | None     # full scope: 数値, path-only: None
    verdict_mismatch_count: int | None  # full scope: 数値, path-only: None

    # component-level match（Path-only scope で有効）
    component_verdict_comparable_count: int  # COMPONENT_VERDICT_COMPARABLE の compound 数
    component_match_count: int               # そのうち component match した数
...
| PR-03 | bridge comparator の component match rate が baseline 閾値以上 | Path-only scope: `path_component_match_rate`（comparable subset 分母）が baseline 以上。full scope: `verdict_match_rate`。v3x_path_verdict_comparability §4.2–§4.3 参照 |
...
bridge comparator の品質を測る baseline は以下で定義する。

**Path-only scope (`comparator_scope = path_only_partial`)**:

| metric | baseline 閾値 | 分母 | 意味 |
|---|---|---|---|
| path component match rate | ≥ 95% | `COMPONENT_VERDICT_COMPARABLE` subset | v3 Path projected verdict が rc2 PAT verdict と一致する割合 |
| coverage drift rate | ≤ 5% | 全 compounds | coverage_drift が発生する compound の割合 |
| metrics drift count | = 0 | Path channel field pairs | deterministic 不一致は許容しない |
| applicability drift rate | ≤ 5% | 全 compounds | applicability_drift が発生する compound の割合 |

**Full scope（将来、`comparator_scope = full`）— provisional rule, full migration ADR で最終凍結する**:

| metric | baseline 閾値 | 分母（provisional） | 意味 |
|---|---|---|---|
| verdict match rate | ≥ 95% | full migration ADR で定義 | v3 shadow verdict が rc2 verdict と一致する割合 |
| coverage drift rate | ≤ 5% | full migration ADR で定義 | coverage_drift が発生する compound の割合 |
| metrics drift count | = 0 | full migration ADR で定義 | deterministic 不一致は許容しない |
| applicability drift rate | ≤ 5% | full migration ADR で定義 | applicability_drift が発生する compound の割合 |

full scope の分母（全 compounds / comparable subset / その他）および閾値は、channel-to-SCV-component mapping の凍結後に full migration contract ADR で最終決定する。上記の閾値は方向性を示す provisional rule であり、Path P6 scope では確定しない。

baseline 閾値は初期値であり、bridge comparator の実運用データに基づいて ADR 改訂で調整する。Path-only scope の baseline は本文書で確定済みである。full scope の baseline は provisional であり、full migration contract ADR で最終凍結する。Path-only scope の `path component match rate` は `verdict match rate` と名称が異なることに注意する（v3x_path_verdict_comparability §4.2 参照）。

### 7. operator-facing report の分離

operator-facing report（`eval_report`, `qc_report` 等）には `semantic_policy_version` を常に表示する（rev.3 §A.2 SM-02, SoT §18.4）。rc2 verdict と v3 shadow verdict を同一 report 上に並べる場合は、以下を遵守する。
...
| 参照元 | 本文書の対応箇所 | 整合性 |
|---|---|---|
| rev.3 §E.2 SCVObservation の `quantitative_metrics` | field coverage table §3–§7 の bridge target 列 | field 名が一致 |
| rev.3 §E.2 `witness_bundle` | field coverage table の `witness_drift` 対象 field | 一致 |
| rev.3 §E.2 `exploration_slice` | field coverage table §7 | 一致 |
| rev.3 §H.5 FB-BRIDGE-01 | ADR-V3-05 §5 の metrics_drift → 調査 action | FB-BRIDGE-01 の input = metrics_drift 蓄積 |
| rev.3 §H.1 FB-SIDE-01 | ADR-V3-06 §3.3 sidecar invariant の特殊位置 | sidecar hook default on 前に required 昇格必須 |
| rev.3 §I ADR-V3-05 | 本文書 ADR-V3-05 | 解決済み |
| rev.3 §I ADR-V3-06 | 本文書 ADR-V3-06 | 解決済み |
| rev.3 §C.6 evaluate returns None | ADR-V3-05 §3.1 coverage_drift / §3.4 applicability_drift | None 時の drift 分類が明確 |
| rev.3 §G.4 goal_precheck run-level 分離 | field coverage table §3 `goal_precheck_passed` → applicability_drift | 一致 |
| path_comparability §1.4 三段階 | ADR-V3-05 §4 CompoundDriftReport.component_comparability | 三段階名が一致 |
| path_comparability §2.4 v3_shadow_verdict = None | ADR-V3-05 §4 CompoundDriftReport.v3_shadow_verdict: None, verdict_match: None | Path-only scope で整合 |
| path_comparability §4.2 comparable subset 分母 | ADR-V3-05 §4 RunDriftReport.component_verdict_comparable_count | 分母が一致 |
| path_comparability §4.3 baseline 読み替え | ADR-V3-06 §6 Path-only scope baseline | 閾値と分母が一致 |
