...
Status: accepted
Date: 2026-04-09
Parent: `adr_v3_10_full_migration_contract.md`, `wp1_wp2_channel_contracts_schema_freeze.md`, `wp6_public_inclusion_decision_memo.md`, `adr_v3_11_m2_authority_transfer.md`
...
- `sidecar_run_record.json` is materialized as the backward-compatible mirror
...
M-2 cutover 後の current runtime では、上記 target field 群を `sidecar_run_record.json.bridge_diagnostics.layer0_authority_mirror.*` から再読し、`verdict_record.json` と field-by-field で照合する。M-1 の表は soak proof と historical replay の authority map として保持する。
...
VN-06 close work は authority transfer そのものではない。M-1 soak が成功しても `verdict_record.json` はなお non-authoritative であり、M-2 cutover は human explicit decision を伴う別 decision とする。この decision は `adr_v3_11_m2_authority_transfer.md` で accepted となり、current runtime は M-2 post-cutover state にある。

## Backward Compatibility

- historical M-1 artifacts では `sidecar_run_record.json` が canonical Layer 0 authority のまま残る
