...
| rc2 Cap 解析（mapping_table, falsification_table） | 統計的証拠 | semantic mapping は UNKNOWN（後述 §mapping framework） |
...
| topic | status |
|---|---|
| Cap → SCV component mapping | UNKNOWN（§mapping framework で扱う） |
...
| offtarget safety | OffTarget Sensor (best_offtarget_distance) | source UNKNOWN（後述 §mapping framework） |
...
| topic | status |
|---|---|
| Catalytic → scv_anchoring mapping の exact projection rule | candidate mapping として定義、freeze は mapping framework gate 通過後 |
| scv_offtarget の v3 source | UNKNOWN |
...
| scv_offtarget | **UNKNOWN** | v3 に OffTarget channel が存在しない |
...
| Catalytic (Rule3A) → scv_anchoring | anchoring sub-evidence → best_target_distance projection | **CANDIDATE** | ADR-V3-04 implementation + 30 run deterministic test |
| ??? → scv_offtarget | UNKNOWN | **UNKNOWN** | v3 OffTarget source decision required |
...
### 4. scv_offtarget source の選択肢

scv_offtarget の v3 source は未決定である。以下の選択肢がある。
...
**Decision**: この ADR では決定しない。scv_offtarget source は UNKNOWN のまま維持し、VN-01（full-SCV mapping frozen）の unmet 条件として残す。Option A/B/C の選択は implementation experience に基づき、後続 ADR で決定する。
...
|---|---|---|
| `sidecar_run_record.json` | canonical Layer 0 sidecar state record | **current authority** |
| `verdict_record.json` | future full verdict record | **schema reservation only** |
| `output_inventory.json` | rc2 inventory | **rc2 authority, unchanged** |

### 2. Migration trigger

`verdict_record.json` が Layer 0 authority に昇格する条件は以下の全てを満たす場合のみである。
...
| channel-to-SCV mapping UNKNOWN | path_comparability §2.2 | 一致（Cap/Catalytic mapping UNKNOWN 維持） |
| VN-01–VN-06 by reference | path_comparability §2.4 | by reference 採用 |
| verdict_record.json schema reservation | path_comparability §2.5 | current = non-canonical 維持 |
...
| channel-to-SCV-component mapping framework | closed (Path FROZEN, Catalytic CANDIDATE, OffTarget UNKNOWN, Cap N/A) |
...
| topic | reason |
|---|---|
| scv_offtarget の v3 source | Option A/B/C の選択は implementation experience に依存 |
| Cap → SCV component mapping | Cap は rc2 SCV に対応 component がないため N/A |
| Catalytic → scv_anchoring mapping の FROZEN 化 | CANDIDATE → VALIDATED → FROZEN の gate 通過が必要 |
| verdict_record.json の exact final schema | full-SCV input coverage 成立後に freeze |
| v3 SCV formula の確定（rc2 と異なる場合） | 別途 ADR |

---

Proposed branch: `docs/v3-wp1-wp2-channel-contracts-schema-freeze`  
Proposed commit: `docs(v3): close WP-1 channel formal contracts and mapping framework, WP-2 schema freeze`
