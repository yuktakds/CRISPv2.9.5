...
def test_run_integrated_v29_full_with_stubbed_core(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run_core_bridge(**kwargs):
        ...
        core_table = write_records_table(out_dir / 'core_compounds.parquet', [{'run_id': out_dir.name, 'molecule_id': 'm1', 'target_id': 'tgt', 'core_verdict': 'PASS', 'core_reason_code': None, 'best_target_distance': 1.0, 'best_offtarget_distance': 5.0, 'final_stage': 1, 'config_hash': 'cfg', 'legacy_core_final_verdict': 'PASS'}])
        evidence_table = write_records_table(out_dir / 'evidence_core.parquet', [{'run_id': out_dir.name, 'molecule_id': 'm1', 'target_id': 'tgt', 'stage_id': 1, 'translation_type': 'global', 'trial_number': 1, 'stopped_at_trial': 1, 'early_stop_reason': None, 'anchoring_witness_pose_json': {}, 'anchoring_fail_certificate_json': {}, 'candidate_order_hash': 'h', 'near_band_triggered': False, 'proposal_policy_version': 'v29', 'config_hash': 'cfg', 'requirements_hash': 'req', 'input_hash': 'inp', 'core_verdict': 'PASS', 'core_reason_code': None, 'legacy_core_final_verdict': 'PASS'}])
        ...
        return CoreBridgeResult(core_rows_path=core_table.path, evidence_core_path=evidence_table.path, diagnostics_path=str(out_dir / 'core_bridge_diagnostics.json'), config_hash='cfg', input_hash='inp', requirements_hash='req', materialization_events=[core_table.to_materialization_event(logical_output='core_compounds.parquet'), evidence_table.to_materialization_event(logical_output='evidence_core.parquet')])
