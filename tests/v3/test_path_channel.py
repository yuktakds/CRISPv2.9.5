...
def test_path_channel_marks_supported_when_blockage_clears_threshold(tmp_path: Path) -> None:
    ...
    assert result.evidence.payload["quantitative_metrics"]["max_blockage_ratio"] == 0.8
    assert result.evidence.payload["quantitative_metrics"]["numeric_resolution_limited"] is None
    assert result.evidence.payload["exploration_slice"]["apo_accessible_goal_voxels"] == 4
    assert result.evidence.payload["witness_bundle"]["witness_pose_id"] == "pose-1"
...
def test_path_channel_marks_refuted_below_threshold(tmp_path: Path) -> None:
    ...
    assert result.evidence.payload["numeric_resolution_limited"] is None
    assert result.evidence.payload["quantitative_metrics"]["numeric_resolution_limited"] is None
...
def test_path_channel_marks_insufficient_for_numeric_resolution_limit(tmp_path: Path) -> None:
    ...
    assert result.evidence.payload["quantitative_metrics"]["persistence_confidence"] == 0.05
